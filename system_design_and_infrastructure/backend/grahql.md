...
To build systems at scale, the `schema` should be split across multiple files/modules. One way of achieving that is using [Apollo Federation](https://www.apollographql.com/docs/federation/).
...
3. Requires new endpoints to be created when adding new query features.

GraphQL solution:

1. GraphQL uses a single API request call to fetch/retrieve the exact data needed and no more.
2. GraphQL uses a single endpoint, and only needs the schema to add a new query. 
