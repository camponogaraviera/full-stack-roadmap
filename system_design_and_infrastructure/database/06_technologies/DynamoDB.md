...
When you perform heavy read or write queries on a database index and exceed the database provisioned capacity, the operation will be throttled, i.e., the rate at which queries are performed will be limited.
This means that requests will be either rejected or slowed down. 
Throttling prevents overuse and increases availability and responsiveness. Nevertheless, capacity units can be set to auto-scale.
