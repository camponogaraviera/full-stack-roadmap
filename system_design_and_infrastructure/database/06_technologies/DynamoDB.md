...
With the new added support for transactional consistency, DynamoDB has become ACID-compliant. CRUD operations are performed through APIs and Tables must be defined a priori. 
...
Suggested video: [AWS re:Invent 2018: Amazon DynamoDB Deep Dive: Advanced Design Patterns for DynamoDB (DAT401)](https://www.youtube.com/watch?v=HaEPXoXVf2k).

Suggested read: [Best practices for designing and architecting with DynamoDB](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/best-practices.html).
...
Any attribute in a DynamoDB table can be projected (used) as a primary key. 

*Projection: ["Represents attributes that are copied (projected) from the table into an index. These are in addition to the primary key attributes and index key attributes, which are automatically projected."](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_Projection.html).
...
A partition key and a sort key can only be of scalar data types: string, number, binary, boolean, and null. When using binary or string as index attributes, they should be <2KB for the hash key (primary key) and <1KB for the sort key. Complex attributes such as `Map` and `List` `are not supported` for use as partition key or sort key.
...
- Query and Scan can only return a maximum of 1MB of data.
...
When creating a DynamoDB table, it is possible to set up custom RCUs and WCUs. When you perform heavy read or write queries on a database index and exceed the database provisioned capacity, the operation will be throttled, i.e., the rate at which queries are performed will be limited meaning that requests will be either rejected or slowed down. Throttling prevents overuse and increases availability and responsiveness. Nevertheless, capacity units can be set to auto-scale.

## Calculating RCUs, WCUs, and Partitions 

- Maximum number of reads per second in a single partition:
  - Strongly consistent read: 3000 RCU in chunks of 4KB.
  - Eventually consistent read: 6000 RCU in chunks of 4 KB.
- Maximum number of writes per second in a single partition: 1000 WCU in chunks of 4KB.

- 1 capacity unity can handle only 1 RPS (request per second).
...
- Write Provisioned Throughput: 1 WCU to insert, delete, and update (projected attributes). 2 WCUs to update items for key attributes.
...
- Write throughput: 1KB * 100RCUs = 100KB/s
