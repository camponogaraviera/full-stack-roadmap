...
Sharding (a.k.a horizontal partitioning) involves dividing a table's rows across multiple tables (partitions or shards) given a partition key (e.g. UserID). Each shard has the same schema but contains a subset of the data. The data in each shard is unique and does not overlap with data in other shards, which helps distribute the load across multiple servers. Sharding is a form of horizontal scaling at the database level, where data can be split across multiple servers or nodes, known as shards, to balance the workload. 

By distributing the data in this way, sharding improves query performance, scalability, and availability, which are key benefits of horizontal scaling.
...
While horizontal scaling is difficult to implement with SQL databases, it is possible in modern MySQL using Vitess, a database clustering system originally developed at YouTube.
