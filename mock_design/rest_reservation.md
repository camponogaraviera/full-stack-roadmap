...
- Set up database replicas for read-heavy workloads reducing hits to a single database server avoiding single point of failures. Use cold/warm/hot standbys. In AWS, [global tables](https://aws.amazon.com/dynamodb/global-tables/) can be used to replicate DynamoDB tables automatically.
- To avoid hotspots when a video goes viral, temporarily store the video in a CDN. Configure a dedicated compute instance to handle peak traffic and sufficient Read Capacity Units to avoid Throttling.
