...
- [Mitigation Strategies for Scalable Systems](#mitigation-strategies-for-scalable-systems)
...
- `Database`: from the CAP theorem, prioritizing [availability and partition tolerance](https://github.com/camponogaraviera/full-stack-roadmap/blob/dev/system_design_and_infrastructure/database/04_CAP_theorem.md) implies that `Cassandra` can be used.
...
1. `Frontend Service:` the UX/UI can be implemented with React Native.
...
7. `Search Engine Service:` the search engine can be implemented with: [Elasticsearch](https://www.elastic.co/elasticsearch), [AWS OpenSearch](https://aws.amazon.com/what-is/opensearch/), and [AWS CloudSearch](https://aws.amazon.com/cloudsearch/).
8. `URL Sharing Service:` users can share their favorite videos using a `URL shortener service`.
9. `Caching Service`: can be used to `reduce latency` by caching the most frequent database queries to the Table attributes that are not likely to change too often. Technologies: [AWS DAX](https://aws.amazon.com/dynamodb/dax/) (purpose-built for DynamoDB), [Amazon Elasticache](https://aws.amazon.com/pm/elasticache/) (general-purpose). This caching solution stays in the AWS cloud.
10. `CDN Service`: can be used to store static content (viral videos) on servers geographically closer to end users to speed up `content delivery` (fast loading times) and mitigate the `celebrity problem`. The CDN takes static content (viral videos) from the [AWS S3 Storage](). Technologies: [AWS CloudFront](https://aws.amazon.com/cloudfront/)(https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/Introduction.html).
...
# Mitigation Strategies for Scalable Systems

1. Deploy the application into multiple node/server instances to ensure high availability and fault tolerance avoiding a single point of failure.

2. Implement a load balancing algorithm to distribute incoming traffic across multiple instances.

3. Implement a caching mechanism to cache the most frequently accessed data to reduce latency and database load.

4. Store static data in a CDN to reduce load on origin servers and speed up content delivery.

5. Set up database replicas for read-heavy workloads reducing hits to a single database server and avoiding single point of failures.

6. Implement database sharding to split table rows across multiple servers improving query performance, scalability, and availability.

7. To avoid hotspots when a video goes viral, temporarily store the video in a CDN. Configure a dedicated compute instance to handle peak traffic and sufficient Read Capacity Units to avoid Throttling.
